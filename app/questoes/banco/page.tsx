...
import type { QuestionStatusFilter } from "@/lib/question-bank";

export const dynamic = "force-dynamic";

const validStatuses = new Set<QuestionStatusFilter>([
  "all",
  "resolved",
  "unresolved",
  "correct",
...
export default async function QuestionBankPage({
  ...
  searchParams: Promise<{ status?: string; subject?: string; lesson?: string; revision?: string }>;
}) {
  const params = await searchParams;
  const initialStatus = validStatuses.has(params.status as QuestionStatusFilter)
    ? (params.status as QuestionStatusFilter)
    : "all";
  const initialSubjectId = params.subject ?? "";
  const initialLessonId = params.lesson ?? "";

  return (
    <PageShell>
      <div className="mx-auto w-full max-w-[1100px] px-4 pb-24 pt-7 sm:px-6 sm:pt-10">
        <Link href={params.revision ? `/revisoes/${params.revision}` : "/questoes"} className="inline-flex items-center gap-2 text-[9px] font-black tracking-[.1em] text-[var(--muted)]">
