...
where onboarding_completed_at is null
  and focus_contest_id is not null
  and active_study_plan_id is not null;
...
alter table public.profiles drop constraint if exists profiles_registration_code_id_fkey;
alter table public.profiles add constraint profiles_registration_code_id_fkey foreign key (registration_code_id) references public.registration_codes(id) on delete set null;
...
values ('theme', '{"mode":"black","accent":"#d6d6d6","accentStrong":"#f2f2f2","surface":"#101113","surfaceRaised":"#17181b","muted":"#92969d","sidebarAura":"#8b8f96"}'::jsonb)
...
insert into public.subject_matrix_settings(subject_id, recommended_weight, minimum_weight, priority_score, incidence_score)
select s.id, 1.00, 0.35, greatest(35, least(90, 85 - ((s.position - 1) * 3))), greatest(35, least(90, 80 - ((s.position - 1) * 2)))
from public.study_subjects s where s.active=true
...
on conflict (subject_slug) do update set image_path=excluded.image_path,accent=excluded.accent,tagline=excluded.tagline,updated_at=now();
...
create index if not exists user_personal_schedule_user_date_idx on public.user_personal_schedule_items(user_id,scheduled_for,position);
...
drop policy if exists registration_codes_admin on public.registration_codes;
...
create policy platform_settings_admin on public.platform_settings for all to authenticated using (public.is_admin()) with check (public.is_admin());
drop policy if exists subject_matrix_read on public.subject_matrix_settings;
create policy subject_matrix_read on public.subject_matrix_settings for select to authenticated using (true);
drop policy if exists subject_matrix_admin on public.subject_matrix_settings;
...
drop policy if exists subject_visuals_read on public.subject_visuals;
create policy subject_visuals_read on public.subject_visuals for select to authenticated using (true);
drop policy if exists subject_visuals_admin on public.subject_visuals;
...
drop policy if exists user_subject_preferences_own on public.user_subject_preferences;
...
drop policy if exists user_personal_schedule_own on public.user_personal_schedule_items;
...
returns boolean language sql security definer set search_path=public,extensions as $$
  select exists(select 1 from public.registration_codes c where c.code_hash=encode(extensions.digest(upper(trim(coalesce(p_code,''))),'sha256'),'hex') and c.active=true and c.uses_count<c.max_uses and (c.expires_at is null or c.expires_at>now()));
$$;
...
create or replace function public.claim_registration_code(p_code text)
returns boolean language plpgsql security definer set search_path=public,extensions as $$
declare v_user uuid:=auth.uid(); v_id uuid;
begin
 if v_user is null then raise exception 'Usuário não autenticado'; end if;
 select id into v_id from public.registration_codes where code_hash=encode(extensions.digest(upper(trim(coalesce(p_code,''))),'sha256'),'hex') and active=true and uses_count<max_uses and (expires_at is null or expires_at>now()) for update;
 if v_id is null then return false; end if;
 update public.registration_codes set uses_count=uses_count+1,last_used_at=now(),active=case when uses_count+1>=max_uses then false else active end where id=v_id;
 update public.profiles set registration_code_id=v_id where id=v_user;
 return true;
end $$;
revoke all on function public.claim_registration_code(text) from public;
grant execute on function public.claim_registration_code(text) to authenticated;

create or replace function public.create_registration_code(p_code text default null,p_label text default null,p_max_uses integer default 1,p_expires_at timestamptz default null)
returns table(id uuid, code text) language plpgsql security definer set search_path=public,extensions as $$
...
begin
 ...
 insert into public.registration_codes(code_hash,label,max_uses,expires_at,created_by) values(encode(extensions.digest(v_code,'sha256'),'hex'),nullif(trim(p_label),''),p_max_uses,p_expires_at,auth.uid()) returning registration_codes.id into v_id;
...
create or replace function public.complete_dynamic_onboarding(p_contest_slug text,p_daily_minutes integer,p_weights jsonb default '{}'::jsonb,p_name text default null)
...
create or replace function public.regenerate_personal_schedule()
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_user uuid:=auth.uid(); v_slug text; v_minutes integer; v_weights jsonb; v_name text;
begin
 if v_user is null then raise exception 'Usuário não autenticado'; end if;
 select c.slug,p.daily_study_minutes,p.nome into v_slug,v_minutes,v_name from public.profiles p join public.contests c on c.id=p.focus_contest_id where p.id=v_user;
 if v_slug is null or v_minutes is null then raise exception 'Onboarding ainda não concluído'; end if;
 select coalesce(jsonb_object_agg(s.slug,usp.weight),'{}'::jsonb) into v_weights from public.user_subject_preferences usp join public.study_subjects s on s.id=usp.subject_id where usp.user_id=v_user;
 return public.complete_dynamic_onboarding(v_slug,v_minutes,v_weights,v_name);
end $$;
revoke all on function public.regenerate_personal_schedule() from public;
grant execute on function public.regenerate_personal_schedule() to authenticated;
