...
export function EnemQuestionSnapshotViewer({
  ...
  const expectedLessonCount = useMemo(
    () => getEnemSnapshotCount(subjectSlug, lessonTitle),
    [subjectSlug, lessonTitle],
  );
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const lessonQuestions = sortEnemQuestionSnapshots(
        manifest.filter((question) => question.lessonKey === lessonKey),
      );

      const uniqueIds = new Set(lessonQuestions.map((question) => question.id));
      if (uniqueIds.size !== lessonQuestions.length) {
        throw new Error(
          `Lista de ${lessonTitle} contém questões duplicadas. A lista foi bloqueada para não esconder nenhuma questão.`,
        );
      }
      ...
      if (sourceSeed) {
        ...
        setSourceAudit({
          label: source.label,
          loaded: lessonQuestions.length,
          total: expectedLessonCount || lessonQuestions.length,
        });
      } else {
        setSourceAudit(null);
      }
      ...
      const visible = savedOnly
        ? lessonQuestions.filter((question) => saved.has(question.id))
        : lessonQuestions;
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Não foi possível carregar as questões desta aula.",
      );
    } finally {
      setLoading(false);
    }
  }, [expectedLessonCount, lessonKey, lessonSlug, lessonTitle, savedOnly, subjectSlug]);
  ...
  async function toggleSaved() {
    ...
    try {
      ...
      if (isSaved) {
        ...
        if (savedOnly) {
          setQuestions((previous) =>
            previous.filter((question) => question.id !== current.id),
          );
          setIndex((value) =>
            Math.max(0, value - (index === questions.length - 1 ? 1 : 0)),
          );
        }
      } else {
        const { error } = await supabase
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Não foi possível alterar a questão salva.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function copyCurrentQuestionImage() {
    ...
    try {
      if (!navigator.clipboard?.write || typeof ClipboardItem === "undefined") {
        throw new Error(
          "Seu navegador não liberou cópia de imagem pela área de transferência.",
        );
      }
      ...
      if (!context2d) {
        throw new Error("Não foi possível preparar a imagem desta questão.");
      }
  ...
  if (loading) {
    return (
      <div className="grid min-h-[420px] place-items-center">
        <LoaderCircle className="animate-spin text-violet-300" size={26} />
      </div>
    );
  }
  ...
  return (
    <div
      className={
        embedded
          ? "w-full"
          : "mx-auto w-full max-w-5xl px-4 pb-24 pt-7 sm:px-6"
      }
    >
      {!embedded ? (
        <Link
          ...
          className="inline-flex min-h-10 items-center gap-2 text-[9px] font-black tracking-[.1em] text-[var(--muted)]"
        >
      ...
      <section
        className={`${embedded ? "mt-0" : "mt-4"} rounded-[30px] border border-violet-400/20 bg-[radial-gradient(circle_at_90%_0%,rgba(139,92,246,.16),transparent_35%),var(--surface)] p-5 sm:p-7`}
      >
        <span className="text-[9px] font-black tracking-[.16em] text-violet-300">
          {eyebrow}
        </span>
        <h1 className="mt-2 font-serif text-3xl text-[var(--ink)] sm:text-4xl">
          {lessonTitle}
        </h1>
      ...
      {errorMessage ? (
        <div className="mt-4 rounded-2xl border border-red-500/25 bg-red-500/10 p-4 text-xs font-bold leading-6 text-red-300">
          {errorMessage}
        </div>
      ) : null}

      {copyMessage ? (
        <div className="mt-4 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 p-3 text-center text-[10px] font-black tracking-[.1em] text-emerald-300">
          {copyMessage}
        </div>
      ) : null}
      ...
      {!current ? (
        ...
        <section className="mt-5 overflow-hidden rounded-[30px] border border-[var(--border-strong)] bg-[#08090b]">
          <header className="flex flex-col gap-3 border-b border-white/[.07] p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-4 text-[9px] font-black tracking-[.08em] transition ${
                  pickerOpen
                    ? "border-emerald-300/35 bg-emerald-400/10 text-emerald-200"
                    : "border-emerald-300/20 bg-emerald-300/[.05] text-emerald-200 hover:border-emerald-200/35"
                }`}
              >
              ...
              <button
                ...
                className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-4 text-[9px] font-black tracking-[.08em] transition ${
                  isSaved
                    ...
                    : "border-white/[.12] bg-white/[.035] text-white/65 hover:border-violet-300/30"
                }`}
              >
                {saving ? (
                  <LoaderCircle size={15} className="animate-spin" />
          ...
          >
            <div className="mb-2 text-center text-[8px] font-black tracking-[.1em] text-black/35">
              BOTÃO DIREITO = COPIAR ESTA QUESTÃO COMO IMAGEM
            </div>
            <svg
            ...
            >
              <image
                ...
                x="0"
                y="0"
                width={current.sheetW}
                height={current.sheetH}
          ...
          <footer className="flex items-center justify-between gap-3 border-t border-white/[.07] p-4 sm:p-5">
            <button
              ...
              className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/[.10] px-4 text-[9px] font-black text-white/55 disabled:opacity-25"
            >
